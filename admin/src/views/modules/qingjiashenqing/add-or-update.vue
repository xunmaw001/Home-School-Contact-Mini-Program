<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info' && !ro.qingjiatupian" label="请假图片" prop="qingjiatupian">
          <file-upload
          tip="点击上传请假图片"
          action="file/upload"
          :limit="3"
        ...
        <div v-else>
          <el-form-item v-if="ruleForm.qingjiatupian" label="请假图片" prop="qingjiatupian">
            <img style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.qingjiatupian.split(',')" :src="$base.url+item" width="100" height="100">
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'" label="教师编号" prop="jiaoshibianhao">
          <el-select :disabled="ro.jiaoshibianhao" @change="jiaoshibianhaoChange" v-model="ruleForm.jiaoshibianhao" placeholder="请选择教师编号">
            <el-option
                v-for="(item,index) in jiaoshibianhaoOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input"v-if="ruleForm.jiaoshibianhao" label="教师编号" prop="jiaoshibianhao">
...
export default {
  ...
  methods: {
      ...
      this.$http({
		...
		if(json.shouji!=''&&json.shouji){
          ...
          this.$message.error(data.msg);
            ...
            this.$http({
              url: `option/jiaoshi/jiaoshibianhao`,
              method: "get"
            }).then(({ data }) => {
              if (data && data.code === 0) {
                this.jiaoshibianhaoOptions = data.data;
              } else {
                this.$message.error(data.msg);
              }
            });
    ...
    // 下二随
    jiaoshibianhaoChange () {
      this.$http({
        url: `follow/jiaoshi/jiaoshibianhao?columnValue=`+ this.ruleForm.jiaoshibianhao,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
        } else {
	...
	this.ruleForm.qingjiashiyou = this.ruleForm.qingjiashiyou.replace(reg,'../../../phpu1613/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
