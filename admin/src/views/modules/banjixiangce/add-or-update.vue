<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="所属班级" prop="suoshubanji">
          <el-select :disabled="ro.suoshubanji" v-model="ruleForm.suoshubanji" placeholder="请选择所属班级">
            <el-option
      ...
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info' && !ro.xiangcetupian" label="相册图片" prop="xiangcetupian">
          <file-upload
          tip="点击上传相册图片"
          action="file/upload"
          :limit="3"
          :multiple="true"
          :fileUrls="ruleForm.xiangcetupian?ruleForm.xiangcetupian:''"
          @change="xiangcetupianUploadChange"
          ></file-upload>
        </el-form-item>
        <div v-else>
          <el-form-item v-if="ruleForm.xiangcetupian" label="相册图片" prop="xiangcetupian">
            <img style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.xiangcetupian.split(',')" :src="$base.url+item" width="100" height="100">
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
      ...
      <el-col :span="12">
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item v-if="type!='info'"  label="相册描述" prop="xiangcemiaoshu">
                <editor 
                    style="min-width: 200px; max-width: 600px;"
                    v-model="ruleForm.xiangcemiaoshu" 
                    class="editor" 
                    action="file/upload">
                </editor>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.xiangcemiaoshu" label="相册描述" prop="xiangcemiaoshu">
                    <span v-html="ruleForm.xiangcemiaoshu"></span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  ...
  methods: {
	...
	this.ruleForm.xiangcemiaoshu = this.ruleForm.xiangcemiaoshu.replace(reg,'../../../phpu1613/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
	...
	if(this.ruleForm.xiangcetupian!=null) {
		this.ruleForm.xiangcetupian = this.ruleForm.xiangcetupian.replace(new RegExp(this.$base.url,"g"),"");
	}
...
var objcross = this.$storage.getObj('crossObj');
    ...
    xiangcetupianUploadChange(fileUrls) {
	this.ruleForm.xiangcetupian = fileUrls;
	this.addEditUploadStyleChange()
    },
	addEditStyleChange() {
	  this.$nextTick(()=>{
