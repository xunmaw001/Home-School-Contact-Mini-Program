<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info' && !ro.tijiaotupian" label="提交图片" prop="tijiaotupian">
          <file-upload
          tip="点击上传提交图片"
          action="file/upload"
          :limit="3"
          :multiple="true"
          :fileUrls="ruleForm.tijiaotupian?ruleForm.tijiaotupian:''"
          @change="tijiaotupianUploadChange"
          ></file-upload>
        </el-form-item>
        <div v-else>
          <el-form-item v-if="ruleForm.tijiaotupian" label="提交图片" prop="tijiaotupian">
            <img style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.tijiaotupian.split(',')" :src="$base.url+item" width="100" height="100">
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
...
export default {
  ...
  methods: {
	...
	this.ruleForm.tijiaoneirong = this.ruleForm.tijiaoneirong.replace(reg,'../../../phpu1613/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
	...
	if(this.ruleForm.tijiaotupian!=null) {
		this.ruleForm.tijiaotupian = this.ruleForm.tijiaotupian.replace(new RegExp(this.$base.url,"g"),"");
	}
...
var objcross = this.$storage.getObj('crossObj');
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `zuoyefankui/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.zuoyefankuiCrossAddOrUpdateFlag = false;
						 this.parent.search();
    ...
    tijiaotupianUploadChange(fileUrls) {
	this.ruleForm.tijiaotupian = fileUrls;
	this.addEditUploadStyleChange()
    },
	addEditStyleChange() {
	  this.$nextTick(()=>{
